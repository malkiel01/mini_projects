...
<div class="card">
  ...
  <form method="get" class="grid" style="align-items:end">
    ...
    <label><span>תוצאה</span>
      <select name="only">
        <option value="">הכול</option>
        <option value="blocked" <?= $only === 'blocked' ? 'selected' : '' ?>>נחסם בלבד</option>
      </select></label>
    <label><span>&nbsp;</span><button class="btn btn--go" type="submit">סינון</button></label>
  </form>
</div>

<div class="card">
  <table>
    <tr><th>מתי</th><th>משתמש</th><th>מה</th><th>כתובת</th><th>תוצאה</th></tr>
    ...
    <tr>
      ...
      <td><?= $a['username']
            ...
            : '—' ?></td>
      <td><?= h($a['kind']) ?></td>
      <td><code><?= h(mb_substr($a['url'], 0, 80)) ?></code></td>
      <td><?= $a['allowed'] ? '<span class="pill pill--ok">הותר</span>'
                            : '<span class="pill pill--stop">' . h($a['code']) . '</span>' ?></td>
    </tr>
    <?php endforeach; ?>
    <?php if (!$rows): ?><tr><td colspan="5">אין רשומות</td></tr><?php endif; ?>
  </table>
</div>
<?php layoutEnd();
